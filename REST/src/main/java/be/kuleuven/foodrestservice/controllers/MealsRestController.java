...
public class MealsRestController {
    ...
    EntityModel<Meal> getBiggestMeal() {
        Meal meal = mealsRepository.findBiggestMeal().orElseThrow(NoSuchElementException::new);
    ...
    EntityModel<Meal> getCheapestMeal() {
        Meal meal = mealsRepository.findCheapestMeal().orElseThrow(NoSuchElementException::new);
